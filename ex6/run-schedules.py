...
def create_table(connection):
    ...
    table_str = "create table grades(id integer primary key, name varchar, grade integer);"
    insert_str = "insert into grades values(1, 'alice', 80), (2, 'bob', 90), (3, 'claire', 100);"
...
def drop_table(con):
    ...
    trans.execute("drop table grades;")
...
def run_scenario_1(con1, con2, level):
    ...
    apply_query(trans1, 1, "select * from grades")
    apply_query(trans2, 2, "select * from grades where id = 1")
    apply_query(trans1, 1, "update grades set grade = grade + 10 where id = 1 returning *")
    apply_query(trans2, 2, "select * from grades where id = 1")
...
def run_scenario_2(con1, con2, level):
    ...
    set_isolation_levels(trans1, trans2, level)
    apply_query(trans1, 1, "select * from grades")
    apply_query(trans2, 2, "select * from grades where grade = 100")
    apply_query(trans1, 1, "insert into grades values(4, 'dan', 100) returning *")
    apply_query(trans2, 2, "select * from grades where grade = 100")
    commit_transaction(con1, 1)
    apply_query(trans2, 2, "select * from grades where grade = 100")
    commit_transaction(con2, 2)
...
def run_scenario_3(con1, con2, level):
    try:
        ...
        set_isolation_levels(trans1, trans2, level)
        apply_query(trans1, 1, "select * from grades")
        apply_query(trans1, 1, "insert into grades select 5, 'trans1', avg(grade) from grades returning *")
        apply_query(trans1, 1, "select * from grades")
        apply_query(trans2, 2, "select * from grades")
        apply_query(trans2, 2, "insert into grades select 6, 'trans2', avg(grade) from grades returning *")
        apply_query(trans2, 2, "select * from grades")
        commit_transaction(con1, 1)
        commit_transaction(con2, 2)
    ...
    apply_query(trans3, 3, "select * from grades")
